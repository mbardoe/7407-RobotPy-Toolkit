...
class PhotonCamera:
    def __init__(
        self,
        name: str,
        poseRelativeToRobot: Pose3d,
        scale_constant: float = 1,
        height=None,
        pitch=None,
    ):
    ...
    def get_scaled_relative_pose(self):
        if self.latest_best_target is not None:
            return (
                self.latest_best_target.relative_pose.translation().toTranslation2d()
                * self.scale_constant
            )

    def refresh(
        self,
    ):  # Call at the beginning of every loop. Saves target for the loop to optimize lookup times.
        self.latest_targets_all = (
            [
                PhotonTarget(target)
                for target in self.camera.getLatestResult().getTargets()
            ]
            if self.hasTargets()
            else None
        )
        self.latest_best_target = (
            PhotonTarget(self.camera.getLatestResult().getBestTarget())
            if self.hasTargets()
            else None
        )
