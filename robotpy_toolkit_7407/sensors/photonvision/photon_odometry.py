...
from robotpy_toolkit_7407.sensors.photonvision.photon_target import (
    PhotonTarget,
    AprilTag,
)
...
class PhotonOdometry:
    ...
    def getRobotPose(self, target: PhotonTarget = None):
        ...
        h = (robot_to_camera.x**2 + robot_to_camera.y**2) ** 0.5
    ...
    def parse_field_layout(self, field_layout: dict):
        ...
        for i in field_layout["apriltags"].keys():
            april_tags.append(AprilTag(ID=i, pose=field_layout["apriltags"][i]))

        length = field_layout["fieldLength"]
        width = field_layout["fieldWidth"]
        new_layout = AprilTagFieldLayout(
            apriltags=april_tags, fieldLength=length, fieldWidth=width
        )
